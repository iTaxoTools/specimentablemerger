...
def gui_main() -> None:
    ...
    if os.name == "nt":
        root.wm_iconbitmap(os.path.join(resource_path, 'data', 'specimentablemerger_icon.ico'))

    style = ttk.Style()
    style.configure("MergeButton.TButton", background="blue")

    banner_frame = ttk.Frame(root)
    logo_image = tk.PhotoImage(file=os.path.join(resource_path, 'data', 'iTaxoTools Digital linneaeus MICROLOGO.png'))
